 ...
 * 
 * The general outline of the algorithm is taken from: <blockquote> <i>Numerical Optimization</i> (second edition) 2006
 * Jorge Nocedal and Stephen J. Wright </blockquote> A variety of different options are available.
 * 
 * <h3>LINESEARCHES</h3>
 * 
 * BACKTRACKING: This routine simply starts with a guess for step size of 1. If the step size doesn't supply a
 * sufficient decrease in the function value the step is updated through step = 0.1*step. This method is certainly
 * simpler, but doesn't allow for an increase in step size, and isn't well suited for Quasi Newton methods.
 * 
 * MINPACK: This routine is based off of the implementation used in MINPACK. This routine finds a point satisfying the
 * Wolfe conditions, which state that a point must have a sufficiently smaller function value, and a gradient of smaller
 * magnitude. This provides enough to prove theoretically quadratic convergence. In order to find such a point the
 * linesearch first finds an interval which must contain a satisfying point, and then progressively reduces that
 * interval all using cubic or quadratic interpolation.
 * 
 * 
 * SCALING: L-BFGS allows the initial guess at the hessian to be updated at each step. Standard BFGS does this by
 * approximating the hessian as a scaled identity matrix. To use this method set the scaleOpt to SCALAR. A better way of
 * approximate the hessian is by using a scaling diagonal matrix. The diagonal can then be updated as more information
 * comes in. This method can be used by setting scaleOpt to DIAGONAL.
 * 
 * 
 * CONVERGENCE: Previously convergence was gauged by looking at the average decrease per step dividing that by the
 * current value and terminating when that value because smaller than TOL. This method fails when the function value
 * approaches zero, so two other convergence criteria are used. The first stores the initial gradient norm |g0|, then
 * terminates when the new gradient norm, |g| is sufficently smaller: i.e., |g| &lt; eps*|g0| the second checks if |g|
 * &lt; eps*max( 1 , |x| ) which is essentially checking to see if the gradient is numerically zero.
 * 
 * Each of these convergence criteria can be turned on or off by setting the flags: <blockquote><code>
 ...
 * </code></blockquote> mem - the number of previous estimate vector pairs to store, generally 15 is plenty. true - this
 * tells the QN to use the MINPACK linesearch with DIAGONAL scaling. false would lead to the use of the criteria used in
 * the old QNMinimizer class.
 * 
...
public class Quasi_Newton_Minimiser
{
	...
	/**
	 * 
	 * The Record class is used to collect information about the function value over a series of iterations. This
	 * information is used to determine convergence, and to (attempt to) ensure numerical errors are not an issue. It
	 * can also be used for plotting the results of the optimization routine.
	 * 
	 * @author akleeman
	 * 
	 */
	...
	public class Record
	{
		...
		public Record(boolean beQuiet, double tolerance, PrintWriter output)
		{
			this.quiet = beQuiet;
			this.TOL = tolerance;
		...
		public Record(boolean beQuiet, double tolerance, double eps)
		{
			this.quiet = beQuiet;
			this.TOL = tolerance;
			this.EPS = eps;
		...
		/*
		 * Initialize the class, this starts the timer, and initiates the gradient norm for use with convergence.
		 */
		...
		/**
		 * This function checks for convergence through first order optimality, numerical convergence (i.e., zero
		 * numerical gradient), and also by checking the average improvement.
		 * 
		 * @return A value of the enumeration type
		 *         <p>
		 *         eState
		 *         </p>
		 *         which tells the state of the optimization routine indicating whether the routine should terminate,
		 *         and if so why.
		 */
		public eState toContinue()
		{
			...
			say(" |" + nf.format(gNormLast) + "| {" + nf.format(relNorm) + "} " + nf.format(Math.abs(averageImprovement / newestVal)) + " ");
	...
	/**
	 * The QNInfo class is used to store information about the Quasi Newton update. it holds all the s,y pairs, updates
	 * the diagonal and scales everything as needed.
	 */
	public class QNInfo
	{
		// Diagonal Options
		 ...
		 * 
		 * This function takes the vector x, and applies the best guess at the initial hessian to this vector, based off
		 * available information from previous updates.
		 */
			...
			switch (scaleOpt)
			{
				...
				case DIAGONAL:
					...
					if (d != null)
					{
						...
						{
							System.err.println(" Vector of incorrect size passed to applyInitialHessian in QNInfo class ");
							System.exit(1);
						}
						// Scale element-wise
		...
		/*
		 * The update function is used to update the hessian approximation used by the quasi newtion optimization
		 * routine.
		 * 
		 * If everything has behaved nicely, this involves deciding on a new initial hessian through scaling or diagonal
		 * update, and then storing of the secant pairs s = x - prevoiusX and y = grad - previousGrad.
		 * 
		 * Things can go wrong, if any non convex behavior is detected (s^T y < 0) or numerical errors are likely the
		 * update is skipped.
		 */
			...
			try
			{
				 ...
				 */
					case SCALAR:
						...
						break;
					/*
					 * DIAGONAL: A diagonal scaling matrix is used as the initial approximation. The updating method
					 * used is used thanks to Andrew Bradley of the ICME dept.
					 */
					case DIAGONAL:
	 ...
	 * computeDir()
	 * 
	 * This function will calculate an approximation of the inverse hessian based off the seen s,y vector pairs. This
	 * particular approximation uses the BFGS update.
	 */

	private void computeDir(double[] dir, double[] fg, QNInfo qn) throws SurpriseConvergence
	{
	...
	public double[] minimize(LogConditionalObjectiveFunction dfunction, double functionTolerance, double[] initial,
	...
	/*
	 * lineSearchBacktrack is the original linesearch used for the first version of QNMinimizer. it only satisfies
	 * sufficient descent not the Wolfe conditions.
	 */
	private double[] lineSearchBacktrack(LogConditionalObjectiveFunction func, double[] dir, double[] x, double[] newX, double[] grad,
	...
	private double[] lineSearchMinPack(LogConditionalObjectiveFunction dfunc, double[] dir, double[] x, double[] newX, double[] grad,
			double f0, double tol) throws MaxEvaluationsExceeded
	{
		...
		do
		{
			...
			if (stage1 && newPt[f] <= bestPt[f] && newPt[f] > fTest)
			{
				...
				infoc = getStep(x, dir, newX, f0, g0, newPt, bestPt, endPt, stpMin, stpMax);
	 ...
	 * 
	 * It was then interpreted from matlab implementation supplied by Andrew Bradley. Modification have been made for
	 * this particular application.
	 * 
	 * This function is used to find a new safe guarded step to be used for linesearch procedures. Comments in the
	 * function were also taken from the matlab implementation.
	 */
	private int getStep(double[] x, double[] dir, double[] newX, double f0, double g0, double[] newPt, double[] bestPt, double[] endPt,
